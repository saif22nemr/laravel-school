         ...
         <div class="card">
            <div class="card-header">
              ...
              <div class="button-control">
                <a href="{{url('/admin/course/create')}}" class="add"><i class="fas fa-plus"></i></a>
                ...
                <div class="row search-form front">
                    <div class="col-sm-6">
                        ...
                        <select name="sortBy" id="sortby">
                            ...
                            <option value="level_title">Level Title</option>
                            <option value="level">Level Number</option>
...
<script type="text/javascript">
  $(function(){ // ready
    ...
    function getcourses(url= apiUrl, filters = {}){
        $.ajax({
            url: url,
            method: 'get',
            ...
            success: function(data){
                ...
                if(courses.length == 0){
                    ...
                    '<th class="text-center"><a>Level Title</a></th>'+
                    ...
                    '<tbody>';
                        ...
                        $.each(courses, function(index,course){
                            rows += '<tr> '+
                                '<td class="text-center"><input type="checkbox" data-id="'+course.id+'" class="checked" name="checked[]" value="'+course.id+'"></td>'+
                                '<td>'+course.id+'</td>'+
                                '<td>'+course.title+'</td>'+
                                '<td>'+course.description+'</td>'+
                                '<td>'+course.levels.title+'</td>'+
                                '<td>'+course.levels.stage+'</td>'+
                                '<td>'+course.levels.level_number+'</td>'+
                                '<td>'+course.created_at+'</td>'+
                                '<td class="text-center control">'+
                                    '<a class="btn btn-info" href="{{url("/course/'+course.id+'/edit")}}"><i class="fas fa-eye"></i></a>'+
                                    '<a class="btn btn-success" href="{{url("/admin/course")}}/'+course.id+'/edit"><i class="fas fa-edit"></i></a>'+
                                    '<a class="btn btn-danger deleteOne" data-id="'+course.id+'" href="#"><i class="fas fa-trash"></i></a>'+
                                '</td>'+
                        '</tr>';
                        });
                        content += rows;
            ...
            error: function(xhr, status, message){
                ...
                console.log(xhr);
                var content = '<h6 style="text-decoration: underline;" class="text-center">Error Get course Table</h6>';
                content+= '<ul>';
    ...
    $('div').on('click','#goNext',function(e){
        ...
        if(currentPage >= lastPage) {
            ...
            getcourses(apiUrl, getFilters(currentPage+1));
        }
    ...
    $('div').on('click', '.deleteOne',function(e){
        ...
        var url = "{{url('/api/course')}}/"+thisItem.data('id');
        $.ajax({
            method: "POST",
            datatype: 'json',
            headers: header,
