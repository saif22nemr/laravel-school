...
$active = ['student', 'form student']; $title = isset($student)? 'Student Update' : 'New student';?>
  ...
  <div class="container-fluid">
    <div class="card add-form">
      <div class="card-header">
        <h3 class="card-title">@if(isset($student)) Update Student @else Add New Student @endif</h3>
      </div>
      <!-- /.card-header -->
      <!-- form start -->
      <form id="form-add" role="form" enctype="multipart/form-data">
        ...
        <div class="card-body">
          ...
          <div class="form-group">
            ...
            <input type="text" <?php if(isset($student)) echo 'value="'.$student->fullname.'"'; ?> class="form-control" name="fullname" id="fullname"  placeholder="Enter student name !!" >
            <div class="alert alert-danger display-none" id="name-message">The name must be not empty</div>
          </div>
          <div class="form-group">
            <label for="email">Email</label>
            <input type="email" <?php if(isset($student)) echo 'value="'.$student->email.'"'; ?> class="form-control" name="email" id="email"  placeholder="Enter student email !!" >
            <div class="alert alert-danger display-none" id="email-message">Invalid Mail</div>
          </div>
           <div class="form-group">
            <label for="username">username</label>
            <input type="text" <?php if(isset($student)) echo 'value="'.$student->username.'"'; ?> class="form-control" name="username" id="username"  placeholder="Enter student username !!" >
            <div class="alert alert-danger display-none" id="username-message">The username must be not empty and must be date</div>
          </div>
          <div class="form-group">
            <label for="birthday">birthday</label>
            <input type="date" <?php if(isset($student)) echo 'value="'.$student->birthday.'"'; ?> class="form-control" name="birthday" id="birthday"  placeholder="Enter student birthday !!" >
            <div class="alert alert-danger display-none" id="birthday-message">The birthday must be not empty and must be date</div>
        </div>
          <div class="form-group">
            <label for="address">address</label>
            <input type="text" <?php if(isset($student)) echo 'value="'.$student->address.'"'; ?> class="form-control" name="address" id="address"  placeholder="Enter student address !!" >
            <div class="alert alert-danger display-none" id="address-message">The address must be not empty</div>
          </div>

          <div class="form-group">
            <label for="phone">Phone</label>
            <input type="text" <?php if(isset($student->phones[0])) echo 'value="'.$student->phones[0]->phoneNumber.'"'; ?> class="form-control" name="phone[]" id="phone"  placeholder="Enter student phone number !!" >
          ...
          <div class="form-group">
            ...
            <input type="file" class="form-control" name="image" id="image"  placeholder="Enter student image !!" >
          ...
          <div class="form-group">
            ...
            <div class="password-generator clearfix">
                <input type="text" class="form-control" name="password" id="password"  placeholder="@if(isset($student)) If you write new password, it will changed @else Enter student Password @endif" >
          ...
          <div class="form-group clearfix">
            ...
            <input type="text" class="form-control" name="password_confirmation" id="password_confirmation"  placeholder="Enter student password confiramtion !!" >
        ...
        <div class="card-footer text-center">
          <input type="submit" class="btn btn-primary text-center" @if(isset($student)) value="Save Updated" @else value="Add New" @endif>
        </div>
      </form>
    </div>
  </div>
...
<script type="text/javascript">
  $(function(){ // ready
    ...
    var editForm = false;
    @if(isset($student)) editForm = true; @endif
      ...
      $.each(input,function(key,value){
        ...
        if(field.prop('name') == 'fullname'){
          ...
          if(field.val().length <= 2){
            ...
            if(compareDate(field.val()) || field.val().length == 0) //if the birthday value is bigger than the crrent date
      ...
      if(submit){ // it will make requests to store new academi year
        ...
        $.ajax({
          ...
          success: function(jsonData, status){
            ...
            if(editForm){
              var content = '<h6 class="text-center">Successfull Update Student '+jsonData.data.username+'</h6>';
            }
            else
              var content = '<h6 class="text-center">Successfull Insert New Student '+jsonData.data.username+'</h6>';
            showNavigator(content);
            //to redirect to academic year list by default
            window.location.replace("{{url('/admin/student')}}");
          },
          error: function(xhr, status, message){ // this error of store academic year
            ...
            console.log(message);
            if(editForm)
              var content = '<h6 style="text-decoration: underline;" class="text-center">Error to Update Student</h6>';
            else
